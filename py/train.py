...
def train(hyp: Dict[str, float]):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(5).to(device)  # mean losses.adaption, original:"mloss = torch.zeros(4).to(device)  # mean losses"
        log.info(
            ('\n' + '%10s' * 9) % 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/roi_net_1_0_0_pre_1000000.weights', help='initial weights path')
