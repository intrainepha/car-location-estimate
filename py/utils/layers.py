...
class ROIInfoConcat(nn.Module): # Add by huyu
    ...
    def forward(self, x, roi):
        if not roi:
            roi = [[0]*4]*x.shape[0]
        roi_tensor = torch.ones(x.shape[0], len(roi[0]), x.shape[-2], x.shape[-1], device=x.device) # batch, depth, hight, width.
        for i in range(len(roi)):
            for j in range(len(roi[i])):
                roi_tensor[i,j,:,:] = roi_tensor[i,j,:,:]*torch.Tensor(np.array([roi[i][j]])).to(x.device)
        return torch.cat([x, roi_tensor], 1)
