...
def resieze_img_and_box(img, box, minPix=100): # adaption
    ...
    min_side = min(h, w)
    if min_side < minPix:
        gain = minPix/min_side
        dst_img = cv2.resize(img, (0, 0), fx=gain, fy=gain, interpolation=cv2.INTER_CUBIC)
        dst_box = [b*gain for b in box]
    else:
        dst_img, dst_box = img, box
    return dst_img, dst_box  
